...
If you don't already run Prometheus and Grafana:
...
Grafana on <http://localhost:3000>, three dashboards already loaded: spend and
latency, audit of everything that left the network, and classifier quality.

Already have monitoring? Skip the profile, copy
`docker-compose.override.example.yml` to `docker-compose.override.yml` to join
your networks, and import the dashboards from `grafana/`.
